...
/**
 * What the harness photographs, and what it stops before it does.
 *
 * Split off `scape-shot.ts` when that file went past the 666-line ceiling, and
 * the seam is the one the tool has always had: this file is the *subjects* —
 * every named pose set, and the list of clocks a capture holds still — and
 * `scape-shot.ts` is the camera that visits them. A run adding a pose set now
 * touches only this file, which is what makes the count of them grow without the
 * harness growing with it.
 */
