...
pack 1200/1709 plates  6.3..21.7m across  standing 0.61m  ice 6.7% of the world  in from freeze 0.49 to 1  up 874 midwinter / 1200 at a hard freeze / 0 midsummer  sheet 0.55  lane 9.1m
...
dune home  crest 2.4m  ridge 8.5m inland  belt 104.8m of coast  2/15 bearings blown out  21% refused  lowest ground 0.003m
dune ridge  crest 2.03m  ridge 6.5m inland  belt 74.9m of coast  2/15 bearings blown out  35% refused  lowest ground 0.04m
dune meadow  crest 2.07m  ridge 7.5m inland  belt 84.2m of coast  6/15 bearings blown out  33% refused  lowest ground 0.025m
dune sound  crest 2.37m  ridge 10.5m inland  belt 223.7m of coast  3/12 bearings blown out  31% refused  lowest ground 0.001m
dune fell  crest 1.91m  ridge 9.5m inland  belt 209.2m of coast  5/15 bearings blown out  58% refused  lowest ground 0.004m
dune shield  crest 1.5m  ridge 9.5m inland  belt 179.8m of coast  6/15 bearings blown out  64% refused  lowest ground 0.004m
marsh ridge  mouth 152°  runs 38.9m of coast  flat 174.1m2 tidal / 140.4m2 turf  gutters 0%  the tide walks 14.5m across it  lowest turf 0.22m
marsh sound  mouth 224°  runs 122.7m of coast  flat 840.6m2 tidal / 704.4m2 turf  gutters 4.4%  the tide walks 17m across it  lowest turf 0.24m
crag home  headland 322°  lip 6.28m  face 69°  runs 45.5m of coast  low 3.23m  1 clefts  stands 5.77m over the coast  plunge 8.95m  coast 0.38
crag fell  headland 218°  lip 9.26m  face 69°  runs 102m of coast  low 3.38m  1 clefts  stands 5.14m over the coast  plunge 6.71m  coast 0.61
stack home  (57.2,-34.6)  off the 328.8° headland  crown 5.95m of a 7m lip  girth 5m  gut 9.5m of water 10.65m deep  clear 5.55m at springs  rock 0.62
stack fell  (185.1,-539.5)  off the 207.4° headland  crown 5.95m of a 7m lip  girth 5m  gut 11.75m of water 15.38m deep  clear 5.55m at springs  rock 0.79
...
moon  phase 0.184  lit 0.3  up 6.9°  lights 0  key share 0  track 1  fire 0
...
`dune home crest 2.4m` is the belt on the weather shore, one line per island that has one. `crest` is the metres of sand standing over the ground at the highest point of the ridge and `ridge` is how far inland of the waterline that point is — a crest that fell to nothing means the ground refused the belt everywhere, which the next number explains. `refused` is the share of the belt's own probes the *ground* turned down, and it is a siting reading rather than a fault: the reach is measured radially and a coast is not a circle, so a belt whose arc lands over a bay asks for a ridge across open water, and one on a steep weather shore asks for sand halfway up a hill. both are refused, which is why `lowest ground` is the invariant to read first — **at or under zero the belt has laid sand in the sea**, and no pose in the tour is pointed at the shore it would be on. `bearings blown out` is how many of the arc's bearings the blowout field has taken under a quarter of the crest; at zero the ridge is an unbroken embankment.

`crag home headland 322°` is the cliff on the steep shore, one line per island that got one — and the islands that got none are the first thing the block says, because a crag is refused a coast that shelves. `coast` is the gradient that won the siting search, in metres of rise per metre inland over the first ten metres of the bare coast: it has to clear `crag.steep`, and a crag whose `coast` reading moved on a run that touched neither the falloff nor the seed has been re-sited. `lip` and `low` are the highest and lowest the drawn cliff top stands over mean water along its own arc, and **they are the claim as a pair** — a cliff line whose two numbers came out equal is a sea wall rather than a headland. `face` is the steepest drawn fall on the rock above the foot, in degrees, and it is the number a screenshot cannot check: a face that quietly became a bank still draws as a dark edge from 1 520 m. `plunge` is the water off the outer edge of the platform, and at or under zero the shelf is a step in a field rather than a cliff. the invariant is silent unless it breaks: the line gains `<- the crag cut Nm out of the island` if the landform ever *lowers* ground, which it cannot do by construction and which is checked anyway, because everything ashore was sited before the rock was raised.

`stack home (57.2,-34.6)` is the pillar the sea left standing off that headland, one line per island that got one — and every line here has to have a `crag` line above it, because a stack cannot exist where a cliff does not. **`gut` is the number to read first, and it is the only one that says this is a stack at all**: it is the submerged run walked back from the foot toward the coast on the *drawn* ground, so at zero the platform has grown out to meet the pillar and what the scape is drawing is a promontory — which is indistinguishable from a stack in every still the tour or any other pose set can take. `depth` is the deepest water in that gap and `clear` is the metres the crown stands over high water at *springs*; at or under zero the rock goes under twice a month, which makes it a skerry, and there are already fifty-nine of those. `crown ... of a ...m lip` is the pair: the share is taken of the level the crag was authored to stand at, the weakness field takes up to a quarter off what gets drawn, and a crown that has reached the lip is a pillar standing over the cliff it was cut out of — the line says so. `rock` is how weak the stone was on the line the sea cut behind, 0..1, off the same field that wanders the lip and punches the clefts: a stack whose `rock` reading moved on a run that touched neither `terrain.crag.geo` nor the seed has been re-sited.

`arch home (33.8,-38.8)` is the hole the sea cut through that headland and has not yet dropped, one line per island that got one — and like the pillar above it, every line here has to have a `crag` line, because an arch is the same cliff one stage earlier. **`headroom` is the number to read first, and it is the only one that says this is an arch at all**: it is the daylight under the middle of the span at high water *springs*, and at or under zero the rock is sitting on the sea, which from every pose this scape is drawn at is the same dark shape as a boulder. `floor Nm wet` is the second half of the same question asked downward — the solve refuses a dry portal against the *bare* seabed and the shore shelving runs after that, so an arch whose hole the drawn terrain filled in is a bridge that passed its own audit, and the line says so in words. `opening` is the clear water between the legs and `Nm of rock over it` is what the span is carrying; the two together are why a `clear` much over three refuses the landform outright rather than drawing a lintel. `crown ... of a ...m lip` is the stack's pair, read the same way, except that this one has a second reading beside it: **the arch's crown has to come out under the pillar's on any headland carrying both**, because the pillar is the later stage of the same rock. `leg stands in Nm` is the drawn water under the outer leg — deeper than `arch.founded` is normal, because the knob is read off the bare bed and this is read off the ground — and `rock` is the weakness the line was cut on, which on a headland with a stack is always the *second* weakest line, never the first.

note that this is the one block in `scape:map` that reports geometry rather than ground. the arch changes no height anywhere, by construction: a height field has one surface per column of air and cannot put rock over water, so the landform is surveyed in `landscape/arch.ts`, drawn in `props/arch.ts` and merged into the steading's hero draw. which means the structural snapshot `scape:diff` compares is *supposed* to be unmoved by a run that adds one.

`marsh ridge mouth 152°` is the tidal flat at the beck's mouth, one line per island whose mouth came out on a coast the sand and the rock had not already taken — and the islands with none are half the archipelago, which is the siting rule doing its work rather than a fault. `mouth` is the bearing, and it is the channel's rather than a search's: silt is delivered, so there is nothing to find. **`the tide walks Nm across it` is the number to read first**, because it is the whole reason the landform exists: the flat's surface stands inside the spring range from end to end, so the sea covers all of it at high water and leaves all of it at low, and this is how far apart those two shores are. under ten metres and the marsh has stopped being flat — the shore the tide poses are taken on shelves at about a metre in four, which 0.4 m of rise walks under two metres, and that was the most any coast in this scape could show before there was a marsh in it. `flat` is the area between low and high springs against the area standing high enough to carry turf, and `gutters` is the share the drainage creeks have cut. `lowest turf` is the invariant, and it is the dune belt's `lowest ground` read from the other end: **at or under `saltings.sward` it is a sward growing in the sea**, and no pose in the tour is pointed at the coast it would be on.

`moon phase 0.184` is the moonlight line, and it is here because every way that system goes quiet looks the same in a still. the moon is a body on an arc, so half of every month it is under the sea at the hour a pose asks for, and a night with no moon up photographs exactly like a night with `daylight.moonStrength` at zero. `up` is its elevation at the parked hour, `lit` the share of the disc the month has left, `lights` the two of them through the astronomical-twilight gate, and **`key share` is the one to read**: how much of the key light the moon has taken, which is how far round the shadows have swung from the bearing the sun set on. the line names both silences — `<- under the sea at this hour` and `<- the moon is up and lighting nothing: moonStrength is at zero` — and a dark night with neither note on it is one the sun has not finished leaving. **`track` and `fire` are the same question asked of the water**, and they are the only reading either half of the night sea has: `track` is the specular budget the key light has on it — 0.41 on the tour's night clock, 1 at any daylight hour, 0 with no moon up — and `fire` is how hard the broken water burns, which is the *complement* of it. a night with a bright track has no fire in it and a new-moon night is all fire and no track, so the two columns summing to more than one is a bug in the coupling rather than a brighter sea.
...
`wreck (-60.1,-56.3) isle 11 freeboard 0.45m` is the hull out on the ring, and it is the one reading in this block that runs the *opposite* way from the line above it. the beacon's freeboard is meant to be large and this one is meant to be small: the search takes the **lowest** qualifying rock on the argument that a rock nobody can see is the rock that catches boats, so **a freeboard that has climbed is the finding** — she has been moved onto a rock a helmsman would have kept off, and in a still that is a hull on a rock either way. `bed Nm of fall` is the second reading: it is the worst the ledge drops away under her bearing, and one that has grown is a hull bridging a hollow rather than aground along her keel. `turn 0deg` says she is still lying on the line she was driven in on; a turn that has gone to the edge of the sweep is a wreck lying square along a skerry, which reads as a boat somebody moored. `wreck NONE` is a real answer and the ordinary one on five of the six islands — only the home island has a ring of rocks at all.

`shoal home (50.1,-14.4) runs 310° for 240m` is the drowned bank trailing off that island's downdrift flank, one line per island the sound leaves room for. **`shelf Nm2` is the reading, and it is the only one that says whether the bank can be seen at all** — it is the area of the bank's own footprint standing in less than `MAX_DEPTH` of water, measured off the composite field rather than off the profile, so anything else that has raised the same ground is in it too. a `shelf` that has collapsed is a bank built under the mask's saturation: still in the field, still in the ferry grid, still on this line, and invisible in every capture ever taken of it. `Nm at low springs` is the other invariant and it runs the other way: **at or under zero the crest dries**, and a bank that dries is an islet, which is a thing nothing in this scape draws — the survey refuses that rather than clamping it, so the line should never show it. `cut short by the sound` says the island opposite took the reach rather than the drift running out, and `shoal ridge NONE` is the refusal, which on this seed is one island in six.
...
bun run scape:shot --poses weir                     # the fish trap on the flat, 4 frames, low water and high
...
`weir` is the set for the fish trap, and it needs two hours for a reason no other structure in the settlement has: the weir is the only thing here that is *not there all day*. twenty metres of wall standing a third of a metre over the bed is a hairline at the tour's 1400 m frame, and the claim the landform makes is the difference between two states of the sea rather than anything in one picture. four frames, on the home island's harbour flat — the one coast in the archipelago that has one: `weir-low` is the trap working, the flat drained and the leader and the ring standing clear of it; `weir-high` is the identical frame with the sea over the top of it, and the pair is the claim — a wall the tide never covered, or one that never showed at all, is two identical pictures. `weir-reach` pulls back to 46 m for the siting, with the boathouse, the bank and the whole course in one frame, and `weir-none` is that same view over the ridge island's harbour, the coast that gets a pier instead. the two hours are the haul-out's own, solved rather than chosen. reach for it whenever the change touches `weir.*`, `tide.range`, the harbour's own arrangement in [`landing.ts`](src/scene/landscape/landing.ts), the shore shelving or the falloff.
...
`drift` is the set for the winter the wind lays, and it is three winters in one frame. the fell island is the subject because it is the tallest ground in the archipelago with no ice cap on it — a run about lying snow cannot be judged on the one island where the white is mostly not snow — and the four frames are all deep winter at noon, pinned for the reason `beck-winter` is. `drift` is the hillside as it now is; `drift-even` is the same frame with `season.snowDrift=0`, which is the winter the scape had and the control the others are read against; `drift-lee` turns the weather right around and has to take the bare ground to the far side of every ridge, which is the dune belt's own argument in snow; `drift-near` is 45 m, where the scoured face and the packed bank either side of one shoulder are surfaces rather than a tone. reach for it whenever the change touches `season.snowDrift`, `season.snowSwing`, `wind.bearing`, the ground normal or the season injection in [`props/material.ts`](src/scene/props/material.ts).

`moon` is the set for the light the moon gives back, and the tour genuinely cannot see it — not for the usual reason. the hour is right: `night` is already pinned at an autumn midnight with a gibbous moon 35° up. the **frame** is wrong. at 1400 m the archipelago is a handful of dark thumbprints under cloud, aurora and stars, moonlight lands on ground, and there is barely a hundred pixels of ground in that picture; it reports 0.11 % changed, which is true and says nothing. so these are the steading's own 48 m frame at the same instant, and they carry **two** controls rather than one, because a night with no moonlight in it has two quite different causes and one appearance. `moon` is the yard as the clock actually has it; `moon-none` is the identical frame at `daylight.moonStrength=0`, which is the night this scape had before the run and the control every claim is read against; `moon-new` is the same yard three weeks on, where the month has put the disc back beside the sun and thirty degrees under the sea with it — it has to look like `moon-none` and it has to do so for a reason in the sky rather than in the config. `moon-reach` pulls back to 90 m for how far the light carries: the ground out to the waterline, and the sound past it. that sound used to be the failure in this frame — the lobe was pointed at the moon and multiplied by a term that was zero at night — and `nightsea` is the set that owns it now. read the stills against `moon` in `scape:map --stats`, which is the only thing that separates the two silences. reach for it whenever the change touches `daylight.moonStrength`, the arcs in [`daylight.ts`](src/scene/daylight.ts), the night half of the sky palette, or anything reading `DaylightState.direction`.
...
`dune` is the newest set, and it is aimed at a *coast*-scaled subject: 105 m of the home island's weather shore, twenty metres deep, on one bearing. every pose in `tour` is aimed at the middle of the archipelago — where a band that width is a few pixels — or at `near`, which is ten metres of open farmyard forty metres inland of it; the run that put the sand on the map moved `tour/default` by 0.14%, and that number is not about the belt. four frames: `dune` is the shore at a 110 m view, holding the ridge, the blowouts through it and the bare lee coast beyond; `dune-lee` is the *identical* frame with `wind.bearing` turned right around, and it is the whole claim as a picture, because a belt on the shore the weather arrives at has to move to the far side of the island when the weather does; `dune-near` is the ridge itself at 40 m, the only frame where a marram tussock is more than a pixel; `dune-winter` is the control the belt needs most, because lying snow whitens the whole island and a pale band that cannot be told from snow is not reading as sand. reach for it whenever the change touches `terrain.dunes.*`, `palette.dune`, the shore shelving, or anything else that repaints or re-levels a coastline.

`crag` is the newest set, and it is aimed at the other coast-scaled subject: 45 m of the home island's steepest shore, fifteen metres deep, at world (39, −30) — the opposite side of the island from the sand, which is the whole point of the siting rule. four frames: `crag` at a 110 m view holds the face, the platform at the bottom of it and the ordinary shelving coast either side, and the contrast between the three *is* the landform; `crag-near` at 38 m is the only frame where a talus block is more than a pixel and the one that shows the platform as a surface rather than as a line; `crag-sea` turns the camera to 135° so the cliff is seen *along* rather than square, which is the view that says whether the lip wanders or runs like masonry; `crag-bare` is the control — the identical frame with `terrain.crag.height=0`, which is the coast this island had before the crag existed. reach for it whenever the change touches `terrain.crag.*`, `palette.seaRock`, `dressing.talus`, the shore shelving, or anything else that re-levels a coastline.

`stack` is the crag set one step further out to sea, and it exists because the run that added the pillars moved every frame in `tour` by between 0.00% and 0.03%: ten metres of rock sixty-seven metres from the world origin is four pixels at the default zoom and nothing at all once the headland is between it and the camera. four frames on the home island's stack at world (57, −35): `stack` at a 90 m view holds the cliff, the gap and the rock as three things rather than one silhouette; `stack-near` at 26 m is the only frame where the taper of the sides and the blocks heaped round the foot are surfaces; `stack-sea` is the same subject from the far quarter, where the pillar comes up against open water instead of against its own headland; `stack-bare` is the control — the identical frame with `terrain.stack.stature=0`. expect the control to sit its whole frame somewhere else rather than merely lose a rock: the focus point is on the pillar, and without it the camera finds ten metres of water there. take it with `scape:shot` rather than `scape:diff`, which hangs on this set the way it does on `crag`. reach for it whenever the change touches `terrain.stack.*`, `terrain.crag.*`, the shore shelving, or anything else that re-levels a coastline.

`arch` is the same coast one stage *earlier* in the sequence, and it needs a set of its own for a reason the stack's does not cover. a pillar is at least a silhouette; an arch is the same silhouette as a lump of rock until the frame is close enough to resolve the opening, and the opening is the whole landform. four frames on the home island's arch at world (35, −40): `arch` at a 40 m view, where the legs, the hole and the cliff behind are three things; `arch-near` at 15 m, the only frame where the soffit curve and the courses of the legs are surfaces; `arch-reach` pulls back to 70 m and is the frame the landform was added for — the headland, the hole cut through its spur and the pillar left where an older hole already fell, all three in one picture; `arch-none` is the control at `terrain.arch.stature=0`, the coast this archipelago had before the run. unlike `stack` and `crag` the focus point is on ground that exists either way, so the control loses a landform rather than relocating its frame. **nothing in this set is turned off the default heading**, and that is deliberate: `rot` does not carry `camera.focusX`/`focusZ` with it, so a rotated frame on a subject fifty metres off the world origin photographs the farmyard instead. read the stills against the `arch` line of `scape:map --stats`, whose `headroom` and `floor ... wet` columns are the two no capture can measure. reach for it whenever the change touches `terrain.arch.*`, `terrain.crag.*`, `quality.archBlocks`, the shore shelving, or anything else that re-levels a coastline.

`saltings` is the newest set, and it is the `tide` set's argument applied to the ground that argument is about. three frames on the sound island's marsh at world (−373, −551): `marsh-low` and `marsh-high` are the identical frame half a cycle of `tide.lag` apart, so the only thing that can have moved between them is the sea — and on a flat whose whole surface stands inside the spring range that water crosses seventeen metres, where the same rise walks under two up the harbour beach `ebb` and `flood` are taken on. `marsh-near` at a 34 m view is the surface itself, and the only frame where the drainage gutters, the bare mud and the cordgrass on the turf are three things rather than one brown patch. the sound rather than the home island because the home island has no marsh: its beck comes out through the dune belt, on the one coast the silt is refused. reach for it whenever the change touches `terrain.saltings.*`, `palette.slob`, `palette.saltings`, `dressing.cordgrass`, `tide.*`, or anything else that re-levels a coastline.

`shoal` is the newest set, and it is the first one aimed at something that has no geometry in it. four frames on the home island's bank at world (103, −77): `shoal` at a 230 m view holds the whole spit and the coast it leaves; `shoal-low` and `shoal-high` are that identical frame half a cycle of `tide.lag` apart at `season: 0.93`, the week the moon makes springs in, which walks 0.8 m of water over a crest that has 0.9 m on it at mean — the tint, the caustics and the surf line all move together and the ground under them does not; and `shoal-none` is the same view at `shoals.reach=0`, which is both the control and the switch, because it is the sea this scape had before the banks and diffs `same` against `origin/main` to the pixel. nothing in `tour` can see any of it: a bank is not small, it is submerged, and at the 1400 m frame it shifts the water tint over two per cent of the sea by a few points of blue under the haze that frame already carries. reach for it whenever the change touches `shoals.*`, `water.surfDepth`, `MAX_DEPTH`, `tide.*`, `wind.bearing`, or anything else that decides what a depth looks like.

`wreck` is the `saltings` set's instrument turned on a *prop*. four frames on the home island's ring at world (−60, −56): `wreck` at a 24 m view reads the hull — the frames, the strakes still on the bedded side, the two broken posts; `wreck-ring` at 96 m holds her low rock and the croft's high one twenty metres along in the same frame, which is the siting's whole argument as a picture, because the search takes the rock nobody chose and the hut beside it is on the rock somebody did; and `wreck-low` / `wreck-high` are the identical frame half a cycle of `tide.lag` apart at `season: 0.93`, which is the week the moon makes springs in — an unqualified year lands on neaps as often as not, and a pair taken at neaps differs by half a metre of nothing. at springs the sea walks 0.8 m up her, and the rock she is on goes from a ledge with a hand's width of weed round it to five centimetres of rock showing. nothing in `tour` can see her: 7.5 m of hull 82 m out is three pixels at the default zoom and off the frame at `near`. reach for it whenever the change touches `wreck.*`, `terrain.isles`, `tide.*`, `palette.wrack`, `palette.driftwood`, or anything else that moves the waterline on the rocks.

`nightsea` is the newest set, and it is the only one whose subject is the *water* after dark. five frames at a 110 m view over the home island's harbour and the open sound beyond it, at world (−34, −44), because the claim needs water with a coast in it — a track needs the sound and the fire needs the surf, and a frame holding only one of them tests half of it. `nightsea` is the tour's own night clock, a waning gibbous 80 % lit and 35° up, which is the moon that lays the track; `nightsea-none` is the identical frame at `water.moonTrack=0`, which is the sea this scape had and the control the track is read against; `nightsea-fire` is the same water three weeks on at a new moon, where the track has nothing to reflect and the glow is no longer quenched — it has to come out *brighter along the surf* than `nightsea`, not darker; `nightsea-dead` is that new-moon frame at `water.phosphor=0`, the picture of a night with neither; and `nightsea-noon` is the same frame at midday, which is the claim that none of this touched daylight and has to read `same` against any reference at any setting of either knob. read the stills against the `track` and `fire` fields of `moon` in `scape:map --stats`, which is the only thing that separates four different black rectangles. reach for it whenever the change touches `water.moonTrack`, `water.phosphor`, `palette.phosphor`, `water.surf`, `water.wakeStrength`, or anything reading `DaylightState.moon` or `DaylightState.dark`.

`shade` is the set for the shadow the cloud deck lays, and it is a 520 m frame over the home island and the sound around it because that is the framing the claim needs: the archipelago is 19 % land, so better than four fifths of a wide frame is water, and the water is the half the shadow never used to reach. `shade` is the scape as the clock has it. `shade-none` is the switch — `atmosphere.cloudShadow=0`, the flat sea and flat ground this scape had — and `shade-clear` is the *second* control and the more interesting one: `atmosphere.cloudCover=0` is a sky with no cloud in it, which used to dapple the whole archipelago anyway, and it now has to come out **identical to `shade-none` byte for byte** rather than merely close. `shade-low` is the projection: the same frame with the sun at the bottom of its arc, where a 34 m deck throws its shadow the better part of two hundred metres downsun. `shade-night` is a moonless midnight, where the cover is up and the darkness is authored and there is no light for a cloud to take away. read the stills against the `shade` line of `scape:map --stats`, whose `dark`, `cover` and `light` columns are the only thing that separates three frames with no dapple on them, and whose `throw` column no capture can measure. **and read the diff at a tighter tolerance than the default**: the shadow is an albedo multiply worth about twenty levels of 255 at its worst pixel, which is well under `pixelmatch`'s perceptual cut at 0.1 — `--tolerance 0.03` is what makes the table say what the pixels already do. reach for it whenever the change touches `atmosphere.cloudShadow`, `atmosphere.cloudCover`, `atmosphere.cloudScale`, `atmosphere.cloudHeight`, `atmosphere.cloudDrag`, the deck in [`clouds.ts`](src/scene/clouds.ts), or anything reading `DaylightState.direction`.

`blow` is the set for the white the open sea puts on, and it is the only set in this file whose subject the tour cannot reach *by construction rather than by framing*. `STILL` zeroes `wind.strength` by definition, so every capture this repository has ever taken was taken in a dead calm — the caps are held at 0.55 of their coverage through it, which is the surf's own argument, but the wind's half of them is invisible to any pose that does not name a wind. these do, which is what `set` running after `STILL` in `shotUrl` is for. **the bracket has to straddle the onset, and that is the trap**: `wind.strength` in the config is the wind *at rest*, and what reaches `capsAmount` is that times the gust front, up to `1 + wind.gust` — a first cut photographed 0.9 against 1.6, which is 1.31 against 2.33 against an onset of 1.25, both over it, both fully capped, and the two frames came out identical to the byte. so `blow-light` is a breeze that barely lifts the sea off its held floor, `blow` is the authored wind, and `blow-hard` is past saturation; the three have to come out monotone. `blow-lee` drops to a 260 m frame under the island's downwind shore, because a lee is a *local* thing and at the wide frames that band is four pixels. `blow-none` is the switch: the identical frame at `water.whitecap=0`, which is the sound this scape had before any of it and must read `same` against the reference. read the stills against the `caps` line of `scape:map --stats`, whose `rest` and `gust` columns are the two no capture can reach. reach for it whenever the change touches `water.whitecap`, `water.whitecapOnset`, `water.whitecapLee`, `water.surf`, `wind.strength`, `wind.gust`, or the bathymetry mask's bearing channels.

`pack` is the set for the ice with a thickness to it, and it is the tour's `winter` frame taken seriously. that frame *can* see the pack — the plates are dealt across the whole archipelago and it opens on the whole archipelago — but at a 1400 m view a fourteen-metre floe is fourteen pixels, and everything the system is actually about is at the edge of one: the rafted ridge, the wet course at the waterline, and the shadow one plate throws over the next. six frames over the sound east of the home island at world (52, −22), which is open water with the bank under it and therefore the first water near the camera's home pose to shut. `pack` at a 90 m view at midwinter is the frame the run was for; `pack-near` at 34 m is the only one in which a single plate is a shape rather than a mark; `pack-reach` at 300 m is the read from the zoom the tour actually uses. `pack-making` is the seasonal claim, at the week the freeze is about seven tenths up — a front rather than a state, with ice on the lobes that close first and open water on the rest. `pack-none` is the switch: the identical midwinter frame at `pack.cover=0`, which is the flat sheet this scape had before any of this and must read `same` against the reference — if it does not, something has moved the *sheet*, which belongs to the water surface and not to this system. `pack-summer` is the other control, the same camera in the week ice has no business existing. read the stills against the `pack` line of `scape:map --stats`, whose `sheet` column is the one thing no capture can check: it is the lowest cover the surface's own `scapeIce` has under any plate at the week that plate arrives, and under `pack.sheet` it means geometry has got out ahead of the shading. reach for it whenever the change touches `pack.*`, `water.iceReach`, `water.iceBreak`, `season.ice`, `palette.ice`, or anything in [`water-ice.ts`](src/scene/landscape/water-ice.ts).
