...
**what a beam is made of is a cone with its colour faded out along it.** additive geometry at a flat tint ends in a visible cone base — a hard-edged plate of light hanging in the dark a hundred metres offshore — and no amount of opacity fixes that, because the edge is in the geometry and not in the blend. the fade is baked into the vertex colours after the merge, so the beam dies into the night at `beacon.beamReach` for free. the material writes no depth and takes no fog: fog would mix a beam toward the fog colour and then *add* it, which puts a grey cone in a hazy sky.
