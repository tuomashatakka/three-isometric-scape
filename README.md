...
**`scape:map --stats` gains a `ledges` line**, because the two things worth knowing cannot be in one frame: `cliffs` against `offered` is the search, and `ashore` at midsummer against `ashore` at midwinter is the year. `ledges 148 birds on 5/5 headlands  4 tiers of a 7m face  band 2.26..6.1m over mean  ashore 148 midsummer / 0 midwinter`, with `<- the year never empties the cliff` when the two counts have come together.
