...
**and the tour could not see any of it — the sixth time this has been written down.** every frame in `tour` is aimed at the middle of the archipelago or at ten metres of home farmyard, and the home island is one of the three with no marsh on it. `--poses saltings` is three frames on the sound's flat at world (−373, −551): `marsh-low` and `marsh-high` are the identical frame half a cycle of `tide.lag` apart, which is the `tide` set's own argument — two frames at two hours of the day differ by the light as well, and the light is the larger signal — and `marsh-near` at a 34 m view is the only one where the gutters, the mud and the cordgrass are three things rather than one brown patch.
