...
the coast between the farm and the water was ground nobody had built on: the yard wants shelter, the pasture wants unused height, the mill wants wind, and every one of those searches walks past the seaward rise. four of the five islands carry a chapel on it now — a limewashed board nave, a west tower with the bell hanging in an open belfry, a shingled spire and a cross, standing clear of every ridge line the island has.

- **two questions asked together, and neither of them is the mill's.** `findChapelSite` gates on `chapel.shore` metres in from the radius that is dry whichever way you walk *and* on `chapel.knoll` metres of prominence. either alone is the wrong building: the shore band by itself lands on the first flat beach, and the prominence by itself takes the island's best hill, which is inland, which is where nobody rowing past can see it. `chapel.knoll` is the switch and there is no boolean beside it — the ridge island, the one island with no shoulder for a mill, has no knoll near enough to its water and gets none
- **the rise is measured by the mill's own function.** `prominenceAt` and `roughnessAt` are exported from `landscape/mill.ts` and now take a `GroundSample` — anything that can be asked how high the ground is — rather than a `MillSearch`. two answers to *is this a rise* is how a chapel ends up in a hollow the mill would have refused; the widening cost one type and no arithmetic, and the mill's own thresholds are untouched
- **the site is a `Standing`, and that is the whole integration.** the five farmstead buildings are placed, faced and walked to through `Standing.angle` and `doorstepOf`; the mill needs a bearing of its own because a mill faces the wind rather than the farm. a chapel is faced like a building, so it is typed like one — and `faceToward`, `doorstepOf`, `planFarmNetwork`, `raiseBuilding` and `paneAt` all took it without a chapel-shaped branch between them. the footpath network plans and costs the leg to its door exactly as it costs the leg to the barn
- **sited last, after the mill, for the reason the mill is sited after the beck.** the beck cannot move, the mill can move a little, the chapel can stand anywhere along a band of coast — so it takes every other feature as a disc or a line to keep off and moves nothing already placed. every mill in the archipelago is on the centimetre it was on
- **the sill is level ground or it is nowhere.** four dry-laid piers tolerate a slope a boarded wall does not, so the roughness gate is 0.9 m over a 2.8 m footing against the mill's 1.3 m over 2.4 m — and it is raised as a `Ploppable` rather than merged like the mill and the light, because a building with a continuous sill wants a foundation that follows the ground rather than daylight under one corner
- **cost is two draws a holding and about three thousand triangles for the archipelago.** 748 triangles a chapel — body and foundation, the same two draws each of the five farmstead buildings already costs — so 40 draw calls became 42 at the pose with one chapel in frame, against a frame of 2.11M triangles. no texture memory, nothing per frame, and no tier gate: a chapel is a building, and every tier that draws a farmhouse draws this
- **glazed out of its own list, by the farmstead's own function.** `glaze` is exported from `props/buildings.ts` and is still the only thing in the scape that turns a `WindowPane` into geometry, so `CHAPEL_WINDOWS` is the survey *and* the source here too. the chapels' panes are **appended** to `surveyWindows`' result rather than interleaved with the holdings': `createWindowLamps` rolls one number per pane in list order, and a pane inserted in the middle relights the farmyards of every island after it. `DWELLING.chapel` is 0.2 against a farmhouse's 1, which leaves three of the archipelago's twenty-four lancets burning at the authored occupancy — lit for a service rather than a sixth outbuilding with lamps in it
- **`scape:map --stats` gained a `chapel` line and the per-island lines gained a column.** `chapel (27.7,-20.5) knoll 4.81m  9.5m in from the coast` — the two halves of the search as numbers, because a chapel that drifted into the middle of the island is a chapel nobody can see from the water, and from every pose the tour takes that looks exactly like a chapel
- **`--poses chapel`, because `tour` could not see this either** — the fifth pose set added for the same reason as the first four. `chapel` moved **31.19% / 99.5% max block**, `chapel-sea` — the frame the siting rule exists for, pulled back over the water — **18.05% / 97.4%**, and `chapel-night` 2.55% / 78.2%. in `tour`, `far` moved 0.36% / 10.4% (the spires, four of them, at 1400 m of view) and `default`, `noon`, `night` and `winter` read `same`
- **`near` moved 15.00%, and it is the ground cover rather than the buildings.** the diff image is the answer: fences, walls, trees and buildings are identical and what is repainted is grass and path stones. a chapel is a new place the farm walks to, the grass rule refuses ground a path already claims, and a new leg is new ground for it to refuse — so the cover re-deals from that point. `STRUCTURE MOVED` is the same fact stated in numbers: `footpaths 16->17` on the home island, and one more leg on each of the three other islands that built one
- **the lamps behind the new glass do not photograph, and neither do the farm's.** at the farmyard at nine in the evening in late autumn, with `windows.occupancy=1` and `windows.glow=6` forced from the url, not one pane draws a visible glow at the `mobile` capture tier — and the identical frame built from `origin/main` is the same, so it is not this change. the chapel's six lancets join a system that is currently dark in a still; worth a run of its own
