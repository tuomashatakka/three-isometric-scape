...
the farm had a mill, a light, a pasture and five buildings round a yard, and nowhere anybody was buried. there is a chapel on the rise above the steading now — a limewashed nave with a stepped chancel, a bell tower with an open belfry and a shingled spire — inside a drystone churchyard with a gate in it and twelve leaning markers in the ground between the wall and the building. four of the five islands build one. a path is worn to its door and its windows are lit after dark.

- **it is the only white building in the scape, and that is the point of it.** falu red is what a farm paints the buildings it walks past every day and the meadow barn is bare because nobody paints a hay store; limewash is the one thing a coast this poor puts on a wall. at the far zoom the steading is a red smudge and the chapel is the thing the eye lands on. the `grime` pass is nearly off on it for the same reason — everything else in the kit darkens toward its base because everything else is worked
- **a chapel is sited by two questions that pull against each other**, and that is the whole difference between [`landscape/chapel.ts`](src/scene/landscape/chapel.ts) and the mill's search next door. it wants to be *seen*, which is a rise, and *walked to* on a winter morning, which is not the best rise on the island. so `findChapelSite` is `findMillSite` with the sign of the distance term flipped, plus one rule the mill has no use for: `chapel.reach` metres from the yard is a **refusal** rather than a penalty, because scoring a headland down merely makes it lose to a better site and nobody carries a coffin up one
- **the first cut measured prominence against the sea, and every shoreline came back a hill.** `prominenceAt` averages a ring twenty-two metres out and subtracts; on a coast this ragged half that ring is water, the seabed is nine metres down, and a spit at the tideline scored 3.95 m. the chapel was duly sited on one, with eight of its fourteen graves in the sea. `landProminenceAt` gives a probe that lands on water the candidate's *own* level, so it neither raises the score nor lowers it: a knoll among land still reads as a knoll and a flat rock in open water reads as flat. the mill is deliberately left reading the raw ground — a headland is exactly where a mill goes, and that drop is the fetch it is sited for
- **the enclosure is probed, not the middle of it**, which is the pasture's lesson applied a second time: a centre being on land says nothing about the ring. `buildStoneWallRun` silently drops any station under `waterLevel + 0.5`, so the failure here would have been a wall with a quarter of its stations missing and nothing saying why. twelve bearings at the wall line, a metre of freeboard each, and the site is refused if any of them is wet
- **the building is the size the island settled, not the size it was drawn.** the first chapel was twelve metres long with a seven-metre footing and an eleven-metre yard, and there is nowhere on the home island to put that: the yard, the pasture, four plots and the mill hold every piece of level ground, and what is left is coast. at 9.6 m long, a 5.5 m footing and a 9 m yard — about a farmhouse — the search finds five squares that qualify and takes the best at 1.56 m of prominence, 46.6 m from the yard
- **the socle is a foundation, not a stilt.** the mill tolerates 1.3 m of fall because a post mill is four dry-laid piers it can pack level; a chapel is a continuous granite course, so the search refuses ground falling more than 0.9 m across half the nave. that is the same number as `PLINTH_REACH` in the dressing, seen from the other end — one decides how much fall a foundation will bridge and the other refuses the ground that would need more. it is also the only outlying building that is *plopped* rather than merged, for exactly that reason: the mill and the light are happy baked at one height, and a nave baked at one height shows daylight under a gable
- **the door is in the west face, and no other building in the kit is.** everything else is modelled fronted on local `+z`, which is what `yawAlong` and `faceToward` assume; a chapel is entered up its axis, so `chapelYaw(bearing) = π - bearing` is the rotation that turns *that* face toward the farm. one function resolves both the standing the dressing raises it on and the standing `windows.ts` carries each pane out through, because a second copy of it is a second chance to get the sign wrong — and a sign wrong there is lamplight on the inside of a wall
- **the chapel is walked to; it is not a place anything else is oriented by.** `footpath.test.ts` caught this and it is the finding worth reading twice. a field's gate is cut toward the nearest *anchor*, and naming the church before the gates were cut turned a plot gate round to face it — which moved that field's only leg onto the cart track, where the tracer correctly refuses to wear a second line beside the road, and left the field with no way in. the waypoint now joins the list after the gates are cut, and `network.test.ts` states that as a fact about the order
- **the walled ground was always one idea, and now it is one module.** the churchyard pushed `dressing.ts` past the 666-line ceiling, and the honest split was not "move the new thing out": the pasture wall, the churchyard wall and the plot fences are the same act — a line traced round some ground, built as one hero geometry, reserved against so the scatter cannot seed through it, left open on one bearing so there is a way in. [`dressing-enclosures.ts`](src/scene/landscape/dressing-enclosures.ts) is that, reached through a `Walling` record rather than by closing over the dressing
- **the markers all face the same way**, on the *chapel's* yaw rather than on their own bearing from the middle: a ring of stones turned to face outward reads as a stone circle, and a churchyard reads as a churchyard because the rows agree. they lie on two arcs in the band between the footing and the wall, starting clear of the gateway so the walk from the gate to the door is not over the plots. one builder rather than two props, with the seed picking a slab or a cross — a yard of only slabs is a rockery and one of only crosses is a decal repeated twelve times
- **cost is eight draws and no texture memory.** four chapels, each a `Ploppable` body plus its foundation skirt, against the material the whole steading already uses. the wall, the gate and the markers merge into the single steading geometry and cost no draw at all; the twenty-eight new panes are twenty-eight more instances in the window-lamp draw that already existed. about 26.5k triangles across the archipelago — 4×1,068 for the buildings, ~4,600 a churchyard wall, ~950 for twelve markers — against the scape's 1.84M, and every tier gets all of it. frame time was **not** measured before and after; the structural budget above is the claim
- **`scape:diff --poses tour`** moved `near` by **31.85% changed / 99.9% max block** — the churchyard wall, two markers and the tower are in that frame — and left `default` (0.13%), `far` (0.41%), `noon` (0.13%), `night` (0.00%) and `winter` (0.02%) alone. `STRUCTURE MOVED` is the footpath count, deliberately: 16 → 18 routes on the home island, and one more on each of the three other islands that built a chapel. a `chapel` pose set was added for the reason `beacon`, `coast`, `steading` and `guard` were — `near` is the tour's only close frame and it is aimed at the farmyard
- **the ref-shot cache could not see a retuned pose, and now it can.** `.ref-sha` keys on the commit and every flag that changes the picture, but a pose is *named* on the command line and *defined* in the working tree — so moving the chapel poses' focus left a `chapel.png` on disk that was a picture from somewhere else, and the diff compared two cameras. each pose's own zoom, rotation, hour, week and overrides are now digested into `.ref-poses` beside the sentinel; a retuned pose rebuilds and the rest of the cache is kept

## the life a bare rock keeps

the guard put forty-nine rocks in water that had been empty and left every one of them a blank grey cap, because `createSpotSampler` draws from the landmass discs and their islets and could not reach one. they carry a tidal band now — bladderwrack in the wash, a strip of bare stone, rock lichen on the crowns that stand clear of it.

- **the zone is one fact, written once.** `config.littoral` is `weedDepth`, `weedRise` and `lichenBase`: how far either side of the waterline the weed holds, and where the crust starts above it. nothing in the section names a skerry, which is what would let the same three numbers dress an island shore the day somebody points them at one. `weedDepth` at 0 collapses the whole band and there is no boolean beside it
- **painted *and* modelled, from the same three numbers.** a rock forty metres across seen from two hundred shows a dark ring at the waterline rather than individual weed, so the ring is a tint in the rock's own vertex colours and the clumps are what the close zoom finds on top of it. both read `config.littoral` — two answers to where the tide reaches is weed hanging in mid-air the first time either moves
- **the paint is a tint and honestly cannot be more.** at the capture tier a skerry patch is three metres to a quad and a rock's exposed cap is about nine metres across, so the cap is three quads wide and a vertex-colour band smears across one interpolation. resolving a sharp tideline means multiplying `SKERRY_DETAIL`, which the guard's own run measured and set at 0.7; raising it to 1.25 cost 30k triangles and did not make the band read, so it went back. the paint does what a low-resolution colour can do and the clumps carry the detail
- **the sampler is the whole reason a budget lands.** `createSkerrySampler` draws from the rocks themselves, area-weighted, and stops at the waterline seam rather than at `Skerry.radius` — the radius runs out to where the stone has fallen back to the seabed and most of that circle is metres under water. forty-nine rocks are a thousandth of a fifteen-hundred-metre field, so a uniform dart lands on one roughly never. `SKERRY_WATERLINE` is exported from `skerry.ts` for it, because a second copy of 0.42 drifts off the stone the first time the profile is retuned. an empty guard returns `null`, and that absence is the second flag there is not
- **the count is a count of the guard, not of an island.** `budget()` grew a `spread` argument. every other scatter takes `areaScale`, the summed island area; sixteen chains are thrown whatever that area is, so a guard budget that took it would have been multiplied by five islands it has nothing to do with
- **cost is two instanced draws and no texture memory.** a wrack clump is five straps, a lichen crust four discs — 4,200 and 2,200 attempts across the whole guard, through `quality.scatterScale`, so `minimal` takes 16% of them and `ultra` 150%. the terrain gains nothing: the paint is a lerp inside a loop that already ran. total triangles at `reef-near` went 2.01M to 2.11M. frame time was **not** measured before and after; the structural budget above is the claim
- **the props are their own file.** `props/littoral.ts`, for the reason `shore.ts` and `upland.ts` are — nothing else in the scape grows where these do. the wrack bends past a half turn so the clump finishes below its own holdfast; the first cut bent 96–148°, read as grass standing in the sea, and `littoral.test.ts` is what caught it, stating the silhouette (broader than tall) as a fact about the geometry
- **`scape:diff --poses guard`** moved `reef-near` by **4.9% max block**, `reef` and `reef-lee` by 2.4%, and `reef-winter` by 0.2% — winter buries the band under lying snow, which is why it is the one guard pose that barely moves. whole-frame `changed` stays under the threshold because the exposed caps are about a hundredth of even the closest frame; `maxblock` is the honest column for a change confined to the rocks. the `tour` set and `scape:map --stats` are unchanged, structure included

## a survey budget the whole suite shares

not a scene change, and it is in this file because it is what made the previous entry's suite go red without anybody touching it. five tests each wrote their own `30_000` for the cost of a second `surveyArchipelago`; the world went on getting bigger every run and that second survey passed thirty seconds, so `scape-map`'s second-seed test failed on main having changed nothing but the size of the world it surveys. the budget is published once now, from `archipelago.ts`, beside the call whose cost it describes — and priced as a hang detector rather than to the current world, because a budget tuned to today's archipelago only schedules the next red suite for whenever the next run adds an island.

## the rocks the open sea never had

the archipelago is fifteen hundred metres across and five patches of it are inhabited. everything between them was one flat seabed quad nine metres down with a lake drawn over it, which at both pulled-out poses is most of the frame and reads as paint. there are forty-nine bare rocks standing in it now, in sixteen chains, and the sea breaks white on whichever side of each one the swell is running at.

- **the second landform built in world space, and the second for the same reason.** the terrain patches deliberately do not overlap, so anything *between* the islands has to be a term added after the composite field has dispatched — exactly as the strand is. [`landscape/skerry.ts`](src/scene/landscape/skerry.ts) folds in as a **maximum**, never a minimum: a rock may raise the seabed it stands on and must never cut into an island it lies off
- **almost none of what a skerry does is written in `skerry.ts`.** the guard is in the one height field and every reader of that field picks it up for nothing — the bathymetry mask bakes off it, so each rock arrives with the depth tint over its shelf and the surf breaking on its weather side; the ferry navigation grid is baked from it, so the boats route around the guard by the clearance test they were already running; `scape:map` samples it, so the rocks draw in ascii without the script being told they exist. there is no rule anywhere that says "avoid skerries"
- **the profile is two curves with the waterline as the seam, and the islets section of the readme is why.** the first cut was one smoothstep dome from the seabed to the crown, which is the mistake that section already records: the seabed is nine metres under the water and a rock carries one or two over it, so the waterline crosses that dome at ninety-two per cent of its height. every rock was a two-metre cap on a forty-metre shoal — and `--stats` reported all of them safely above water, because at the centre they were. `SHELF` puts the seam at 42% of the radius. neither half is a smoothstep, because two smoothsteps either side of the seam would put a bench of dead-level ground exactly at sea level round every rock, and the water plane is drawn over it. `skerry.test.ts` is what found it, walking eight bearings out to a quarter of the radius rather than asking the centre
- **where the rocks go is a rejection, not a construction.** a chain picks a head, walks a wandering bearing and drops a rock every `spacing` metres; a step that lands on an island, on the bar or on a rock already there is skipped rather than nudged somewhere legal. so the count is an outcome — sixteen chains asked for, sixteen taken, forty-nine rocks — and `--stats` reports both, because a run that quietly halved the reef would look identical at every pose but the one nobody takes
- **`skerries.crest` is the switch and the only one.** zero drowns the whole guard and the open sea is open sea again; there is no boolean beside it. every length in the section is metres and stays metres — the one world-sized number is the domain the chains are thrown at, and that is read from `archipelago.worldSize` rather than written down. build-time like `strand.*` and `creek.*`, so it is deliberately not in the overlay
- **the ferries are unchanged, and that is stated rather than screenshotted.** `createWaterways` throws rather than sail a boat through a rock, so `skerries.clearance` at 70 m — comfortably more than `boats.routeCell` — keeps every landing's water several navigation cells wide before the planner ever runs. five legs, 2,516.7 m of route and 0.55 m of minimum clearance, identical to `main` with forty-nine rocks in the field, and `skerry.test.ts` asserts it
- **cost is no draw call and no material.** each rock is a small patch sampled from the composite field and merged into the same single terrain geometry the islands and the bar are in. `SKERRY_DETAIL` is the outer fells' argument at a different number: 0.45 was the first cut and `--poses guard` refused it — five and a half metres to a quad against the home island's two, and the crowns read at `reef-near` as faceted lumps rather than rock — so it is 0.7, about 22k triangles for the whole guard against the scape's 1.84M, with the tier's own `terrainSegments` still scaling it underneath. no texture memory, nothing per frame, and every tier gets the rocks
- **the rocks are bare, and that is the sampler rather than a rule.** `createSpotSampler` draws candidates from the landmass discs and their islets only, so no scatter budget can reach a skerry — no reeds growing in the middle of the ocean, and no cost. lichen on a guard would want the sampler told about the rocks, which is a run of its own
- **`scape:map --stats` gained a `skerries` line.** `skerries 49 in 16 guards  widest 22m  lowest 0.8m over the water  nearest island 79.9m` — the count against the chains, the freeboard of the worst rock read off the composite field, and the clearance the ferry planner is relying on. a rock at or under zero is a shoal, and the line says so

## the light the windows kept

every farmhouse, barn and sauna in the archipelago was modelled with glass in it, and every pane of it was the same cold grey at midnight as at noon. sixty-five of them are lit now — warm at dusk, banked to a stove glow once the household turns in, and up again before dawn, while the lighthouse on the outer rock burns straight through, because a lighthouse is a machine and a farm is not.

- **a pane is published, not measured.** `FARMHOUSE_WINDOWS`, `BARN_WINDOWS` and `SAUNA_WINDOWS` are exported from [`props/buildings.ts`](src/scene/props/buildings.ts) and the builders glaze themselves *out of* them — `glaze()` is now the only thing in the file that calls `window()`. the same seam the chimneys are on, and for the same reason: two answers to where a window is, is a lamp hanging in a wall the first time either moves. the dormer stays out of the list and stays dark, because `dormer()` derives its own pane from the roof plane it is handed and a copy of that arithmetic would drift the first time the pitch did
- **the gables are glazed, because the camera has one heading.** a nine-metre farmhouse with glass only in its two long walls turns a blank end to the eye on half the yaws the reader can spin the scape to, and a lamp nobody can see from the angle the scape opens at is a lamp that may as well not be lit. `window()` in [`props/timber.ts`](src/scene/props/timber.ts) grew a wall axis — `x` for a gable, `z` for a long wall, a named wall rather than an arbitrary rotation — and the farmhouse and the barn each gained a pane in either end. thirteen panes a holding rather than nine, and the same single draw
- **where the panes end up is a survey.** [`landscape/windows.ts`](src/scene/landscape/windows.ts) is pure the way `hearths.ts` is, and the transform they both need — a point in a raised building's own frame, in world metres — moved into the new [`landscape/fixtures.ts`](src/scene/landscape/fixtures.ts) rather than being written twice
- **the outward normal is the trap.** a pane on the far wall looks the other way, so its bearing is the standing's turned half a circle, and the mirrored version paints the glow on the *inside* face where the building hides it — which from the default pose is indistinguishable from the lamps not working at all. so each pane carries its own building's placed centre on the record, and both `windows.test.ts` and `scape:map --stats` state the sign as a fact about the data rather than re-running the arithmetic
- **two questions, not one.** the sun says whether a lamp is wanted — `(1 - day)²`, the same dusk curve the lighthouse comes up on, so two lights on one coast cannot disagree about when dusk is. the clock says whether anybody is up to have lit one. `windows.banked` is why a sleeping farm is a stove glow rather than black: a scape whose farms go absolutely dark at four in the morning reads as abandoned rather than asleep. a `bedtime` at or before `rising` is a household that never gets up, and the test says so
- **which windows are occupied is a live knob.** the draw is made once per pane at build and *kept*; the comparison against `windows.occupancy` happens every frame, so turning the farm up at midnight is a slider rather than a reload — and because the roll is weighted per building, it lights the farmhouse before the sauna and the sauna before the byre. all seven knobs in the new `windows` section are read per frame, so all seven are in the overlay
- **the glow is sized in pane units, and there is no length in the config at all.** the carrier scales each instance by its own window's width and height, so the haze grows with the opening it comes out of — a bigger window lets out more light. writing the spill in metres instead needs one geometry per pane size, which is a draw call apiece for a difference nobody can see. that is the whole scale audit, written at the knob
- **cost is one instanced draw for the archipelago, skipped outright while the sun is up.** sixty-five instances of a two-triangle pane plus a radial haze fan — 74 triangles an instance at desktop's three rings, 2 on `minimal` — which is 4,810 and 130 triangles across the whole archipelago. no texture memory, no allocation per frame, and one colour write per pane only on a frame the flicker actually advanced — which with `windows.flicker=0`, how every capture is taken, is never. `quality.lampSpill` is 0/2/3/4 by tier, so the cheapest device gets a lit window on a clear night rather than a coarse glow
- **`windows.flicker` is a new line in `STILL`**, and the rate pattern in `scape-shot.test.ts` grew `flicker` so the next one cannot be forgotten. `--poses steading` gained a fourth frame, `yard-evening` at nine in the evening: `yard-night` is half past midnight, which is deliberately after the household has turned in, and a pose that only ever saw the lamps banked could not tell a farm with people in it from an empty one
- **the tour reports `same` at all six poses, and that is the instrument rather than the change.** every pose in `tour` is aimed at the middle of a 1520 m archipelago, where a 0.8 m pane is a third of a pixel — the same reason the smoke and the surf needed sets of their own. `--poses steading` is where it shows: `yard-evening` 0.07% changed / **12.3% max block**, `yard-winter` 0.04% / 9.4% (midwinter at 68° north is a polar night, so the lamps burn at midday), `yard-night` 0.01% / 1.4% (the banked stove glow), and `yard` in daylight 0.00% / 0.6% — which is the new gable glass and nothing else. `scape:map --stats` is unchanged in every field but the new one
- **`scape:map --stats` gained a `windows` line.** `windows 65  lowest pane 1.85m over the ground  facing out 65/65` — the count, the chimney clearance check from the other end of the building, and the sign check. anything short of the full count on `facing out` is a lamp lit on the inside of its own wall

## the smoke the chimneys never made

every farmhouse in the archipelago was modelled with a brick chimney and every sauna with an iron flue, and not one of them had ever been lit. all ten of them smoke now — a column that leans on the same one wind as the grass and is banked harder the colder the week.

- **the mouth is the prop's own answer, not a second copy of it.** `FARMHOUSE_CHIMNEY` and `SAUNA_FLUE` are exported from [`props/buildings.ts`](src/scene/props/buildings.ts) and the masonry is now placed *from* them — the cap course sits at `mouth.y - thickness/2` rather than beside a repeated `-2.6`. the same seam the lantern hubs and the mill's wheels are on, and for the same reason
- **where the stacks stand is a survey, not geometry.** [`landscape/hearths.ts`](src/scene/landscape/hearths.ts) is pure and testable the way `colony.ts` is: it probes the four corners of a building's claim for the floor it will be levelled onto, then carries the mouth out through the same `rotateY` the building is raised with. the sign is the trap — a mirrored yaw looks right on every seed where a farmhouse faces square and puts the smoke out of the gable end on the rest, so `hearth.test.ts` states it as a fact about the data
- **a column is a queue, not a crowd.** each puff takes a fixed slot along one shared climb, so `quality.hearthPuffs` buys *continuity* rather than reach and only the character of a puff comes out of the rng. the billboard is built in view space rather than about the vertical axis, because the camera's yaw is a live control and a `y`-billboard shears visibly as the scape is turned
- **the wind is a response, not a rate.** `hearth.drag` is dimensionless and multiplies the scape's one wind, the way `atmosphere.cloudDrag` and `mistDrag` do; the lean goes as the square of the age, because a puff keeps what it has already been given. the wander across the wind is carried by `wind.travel`, so it stops when the wind does — and `hearth.speed`, which is heat rather than weather, is its own new line in `STILL`
- **one winter, not two.** `hearthDensity` reads `season.growth` — the same instant of the year the grass is withered by — rather than carrying a curve of its own, and the tuning holds midwinter at 0.99 so the clamp at a fully opaque plume is a guard rather than a working range
- **cost is one draw call for the whole archipelago.** one program, one static buffer, no allocation per frame; four vertices and two triangles a puff, so desktop's ten stacks at fourteen puffs are 560 vertices — less geometry than a single spruce. no texture memory. `minimal` gets none and `mobile` gets eight puffs a stack, which is the graceful absence and the cheap-but-real version rather than a broken one
- **`--poses steading`, because `tour` could not see this either.** a plume is three pixels at the default pose, and `near` — the tour's one close frame — is focused on the world origin, which is open yard between the farmhouse and the sauna and takes neither in. the new set is three frames on the farmyard at a 48 m view: `yard` moved 0.54%, `yard-winter` 1.19% (the banking claim as a picture) and `yard-night` least of all, because the smoke is dimmed with the day. same fix, and the same reason, as `--poses beacon` and `--poses coast`
- **`scape:map --stats` gained a `hearths` line.** `hearths 10  lowest mouth 5.54m over the ground` — the count, and the tightest clearance between any mouth and the ground under it. under about three metres means a stack has been placed against a floor it does not stand on, which is invisible in a still
