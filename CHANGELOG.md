...
- **cost is no draw call and no material.** each rock is a small patch sampled from the composite field and merged into the same single terrain geometry the islands and the bar are in. `SKERRY_DETAIL` is 0.45 — the same number and the same argument as the outer fells' — so the guard is roughly 10k triangles against the scape's 1.84M, and the tier's own `terrainSegments` still scales it. no texture memory, nothing per frame, and every tier gets the rocks
