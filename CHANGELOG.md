...
every farmhouse, barn and sauna in the archipelago was modelled with glass in it, and every pane of it was the same cold grey at midnight as at noon. forty-five of them are lit now — warm at dusk, banked to a stove glow once the household turns in, and up again before dawn, while the lighthouse on the outer rock burns straight through, because a lighthouse is a machine and a farm is not.
...
- **`scape:map --stats` gained a `windows` line.** `windows 45  lowest pane 1.85m over the ground  facing out 45/45` — the count, the chimney clearance check from the other end of the building, and the sign check. anything short of the full count on `facing out` is a lamp lit on the inside of its own wall
