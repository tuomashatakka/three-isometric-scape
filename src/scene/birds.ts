...
export interface BirdOptions {
   ...
   *
   * A gull holding station over a harbour works for it, and works harder the
   * harder it is blowing — so the wingbeat takes the wind the way the mill's
   * sails do. Only the beat: the wheel is the bird choosing where to go, and a
   * flock that stopped turning on a still day would be a flock that had landed.
   */
  wind: WindState
}
...
/**
 * How much harder a bird beats its wings for the wind it is holding against.
 *
 * A share rather than a multiplier on the whole rate, so `birds.flap` stays the
 * bird's own rate and this is only the effort on top of it — which also keeps
 * `birds.flap=0` the one thing that stops the wings, whatever it is blowing.
 */
export function beatEffort (windStrength: number): number {
  return 0.72 + 0.28 * Math.min(2, Math.max(0, windStrength))
}
...
export function createBirdFlocks ({
  ...
  return defineModule<ScapeConfig>({
    ...
    update (_state, frame) {
      ...
      beaten = (beaten + frame.delta * birds.flap * TAU * beatEffort(wind.strength)) % TAU
