...
// draws with one.
//
// three links every program up front — `createApp` calls `renderer.compile`
// on mount — but it does not *check* the link until the program's first use
// (`WebGLProgram.js`, `onFirstUse`). By then the broken program is already
// bound to a draw, and on the handset this repo has been chasing that is the
// whole failure: a program that will not link is bound anyway, every draw
// against it raises INVALID_OPERATION, and ANGLE eventually takes the context
// away. The loss lands seconds after load, at whatever moment the offending
// material first came into view, which is why it read as thermal for four
// rounds of fixes.
//
// Reading LINK_STATUS ourselves, after compile and before the first frame,
// turns that into a blank screen with a material name on it. Nothing draws, so
// nothing cascades, so no context is lost — and the answer arrives at t=0
// instead of never.
...
export const SCAPE_FAMILIES = [
  'inject', 'detail', 'dressing', 'water', 'mist', 'clouds', 'aurora', 'rain', 'post', 'shadows',
] as const
...
/** A program the driver refused, and everything worth knowing about it. */
...
/** Where the audit says its piece — the `ScapeDiagnostics` shape, narrowed. */
...
/**
 * Read every program's verdict out loud, and say whether the scape may draw.
 *
 * `true` means keep the loop stopped. That is the entire fix for the context
 * loss: the draws are what turn a refused program into a dead context, so a
 * scape that never starts cannot lose one — and the reader gets the failing
...
export function reportPrograms (renderer: WebGLRenderer, report: AuditReport, auditOnly: boolean): boolean {
  ...
  if (faults.length) {
    ...
    // the cause of a loss, which no run before this one has been able to do.
