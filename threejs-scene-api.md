...
| `readPath` | function | src/ui/settings-store.ts, src/ui/settings-store.test.ts, src/scene/state-path.test.ts +1 |
...
| `Store` | interface | src/context-recovery.ts, src/scene/config-access.ts, src/scene/create-isometric-scape.ts |
| `StoreListener` | type | — |
